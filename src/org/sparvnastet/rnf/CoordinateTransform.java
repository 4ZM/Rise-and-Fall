...
public class CoordinateTransform {
    private float xs_, ys_, xo_, yo_;

    public CoordinateTransform(float xScale, float yScale, float xOffset, float yOffset) {
        xs_ = xScale;
        ys_ = yScale;
        xo_ = xOffset;
        yo_ = yOffset;
    ...
    public Vec2 transform(Vec2 p) {
        if (p == null)
            throw new IllegalArgumentException();

        return new Vec2(p.x * xs_ + xo_, p.y * ys_ + yo_);
    }
}